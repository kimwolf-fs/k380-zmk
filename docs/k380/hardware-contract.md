...
**验证方式：** 对每项执行可重复的实板操作并记录 PCB revision、测试日期、固件版本、测量值、枚举结果和异常项。矩阵验证必须使用可观察 `(row, column)` 的诊断固件，覆盖 80 个有效按键、40 个未使用坐标、非歧义多键、矩形歧义、保持/释放以及真实 P0/P1 GPIO 中断唤醒、扫描停止和异常恢复；如发现 GPIO 或 RC 错误，先更新对应唯一来源文档。LED 检查必须覆盖所有蓝牙状态、所有 LED4 事件、优先级和 USB 接入一次性提示。矩阵诊断固件的 J-Link RTT 操作流程见 [`matrix-rtt-diagnostics.md`](matrix-rtt-diagnostics.md)。
