...
| H10 | 分别用 Z5、Z7、低压唤醒源唤醒 | 每个 system-off 唤醒源均首键只唤醒；建立 BLE 连接后第二键才输出 | Z7 分支：切换槽位后蓝灯配对，约 60 秒进入休眠；唤醒后仍处于配对状态，首键无输入。因未建立主机连接，第二键无 HID 输出无法判定 | Z7 唤醒状态通过；输入项待连接主机复测 |
...
- 判断：Z7 配对状态恢复符合预期；未建立 BLE 主机连接时不能用 HID 输出判定第二键语义，需在配对主机成功连接并显示绿色后再按普通键确认
