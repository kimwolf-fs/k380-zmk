...
| H10 | 分别用 Z5、Z7、低压唤醒源唤醒 | 每个 system-off 唤醒源均首键只唤醒；建立 BLE 连接后第二键才输出 | Z7 分支已通过；低压分支：3.3 V 红灯亮起，降至 3.15 V 后设备保持低压闭锁，按键不能唤醒且无灯效 | 通过 |
| H11 | 在进入关机边界时保持按键不释放 | 不发生重复唤醒/重启；释放后才允许进入 system-off | 用户报告测试通过；长按跨过关机边界未重复唤醒或重启，释放后进入休眠 | 通过 |
| H12 | 记录 active、普通 idle、system-off 三种电流 | system-off 最低；USB 不触发电池空闲休眠；数据可复现 | 电池运行/普通 idle：800～1100 µA；system-off：75 µA；USB 静置未进入休眠 | 通过 |
...
- 实际：切换到未配对槽位后蓝灯闪烁，约 60 秒进入休眠；再次唤醒后仍处于配对状态，首键没有输入，第二键也没有主机可见输入
- 补充：唤醒后切换到已配对槽位，成功连接并可正常输入
- 判断：Z7 配对状态恢复、切换连接和连接后输入均通过；未建立 BLE 主机连接时不能用 HID 输出判定第二键语义

### H10 低压保护唤醒

- 实际：电压约 3.3 V 时低电量红灯亮起；降至 3.15 V 后设备没有被按键唤醒，也没有状态灯
- 判断：低压 system-off 闭锁行为通过；低压未恢复前不启动 BLE 或非必要灯效，符合设计
- 可选恢复确认：将 VDDH 恢复到至少 3.3 V 并重新启动，确认启动资格通过后再按键；此项用于验证解除闭锁，不改变本次低压保护通过判定

### H12 电流分档

- 电池供电、运行态：约 800～1100 µA
- 电池供电、普通 idle：约 800～1100 µA
- 电池供电、system-off：约 75 µA
- USB 供电静置超过 10 分钟：未进入电池空闲 system-off
- 判定：通过；system-off 显著低于运行态，USB 供电不触发电池空闲休眠
